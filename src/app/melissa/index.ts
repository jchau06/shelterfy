...
const getLocation = async(addrLineOne:string, country:string, zipCode:string, locality:string) => {
  try {
    ...
    return findNearbyLocations(parseFloat(jsonParsedResp["Records"]["Latitude"]), parseFloat(jsonParsedResp["Records"]["Longitude"]));
  } catch (error) {
...
const verifyBuisness = async (addrLineOne:string, zipCode:string, city:string, state:string="CA", country:string="US") => {
  try {
